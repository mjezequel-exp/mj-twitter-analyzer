...
import { ref } from "vue";
import { azureOpenAIService, type CompleteAnalysis } from "../services/AzureOpenAIService";
...
const tones = azureOpenAIService.getAvailableTones();
...
const analyzeMessage = async () => {
    ...
    try {
        analysis.value = await azureOpenAIService.analyzeMessage(message.value);
...
const modifyTone = async (tone: string) => {
    ...
    try {
        const modifiedResponse = await azureOpenAIService.modifyResponseTone(analysis.value.suggestedResponse, tone);
            ...
            <q-card class="q-pa-md q-mb-md" flat>
                <div class="row items-center justify-between">
                    <div class="text-h6 text-white text-weight-bold">
                        ...
                        Analyseur de Messages Twitter
                    </div>
                    <div v-if="isAuthenticated" class="row items-center q-gutter-md">
                        <div class="text-body2 text-white">Bonjour {{ accounts[0].name }}</div>
                        <q-btn @click="logout()" outline color="white" label="Déconnexion" icon="logout" no-caps size="sm" />
                    </div>
                    <div v-else>
                                    ...
                                    <div class="col">
                                        ...
                                        <div v-if="analysis.toxicity.types.length > 0" class="q-mt-md">
                                            <div class="q-gutter-xs">
                                                <q-chip v-for="(type, index) in analysis.toxicity.types" :key="index" size="sm" color="red-2" text-color="red-8" dense>
                                                    {{ type }}
                                                </q-chip>
                                            </div>
                                        </div>
